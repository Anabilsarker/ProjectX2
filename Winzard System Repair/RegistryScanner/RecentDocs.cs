    ...
    public class RecentDocs : ScannerBase
    {
            ...
            try
            {
                using (RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\ShellNoRoam\MUICache"))
                {
                    ...
                    foreach (string valueName in regKey.GetValueNames())
                    {
                        ...
                        ScannerFunctions.CurrentScannedObject = valueName;

                        if (!Utils.FileExists(valueName))
                            ScannerFunctions.StoreInvalidKey(Strings.InvalidFile, regKey.Name, valueName);
                    }
                }
            }
        ...
        private static void EnumMRUList(RegistryKey regKey)
        {
            foreach (string strValueName in regKey.GetValueNames())
            {
                ...
                {
                    ScannerFunctions.StoreInvalidKey(Strings.InvalidRegKey, regKey.ToString(), strValueName);
                    continue;
                }
                else
                {
                    ...
                    ScannerFunctions.CurrentScannedObject = shortcutPath;
                    ...
                    {
                        ScannerFunctions.StoreInvalidKey(Strings.InvalidFile, regKey.ToString(), strValueName);
                        continue;
                    }
                }
            }
